...
void execute_command(const int command_index, char *command_line,
					 galaxy_t **galaxy, int *global_size)
{
	...
	case 0:
		sscanf(command_line, "%s%s%d%d", received_command, planet_name,
			   &planet_index, &shields_number);
	...
	case 1:
		...
		break;
	case 2:
		sscanf(command_line, "%s%d%d%d", received_command, &planet_index,
			   &shield_index, &upgrade_value);
		UPG(planet_index, shield_index, upgrade_value, galaxy);
		break;
	case 3:
		sscanf(command_line, "%s%d%d", received_command, &planet_index,
			   &shield_value);
	...
	case 5:
		...
		break;
	case 6:
		sscanf(command_line, "%s%d %c%d", received_command, &planet_index,
			   &direction, &units);
...
void ADD(char *planet_name, const int planet_index, const int shields_number,
		 galaxy_t **galaxy, int *global_size)
{
...
// Upgrade a planet
void UPG(unsigned int planet_index, unsigned int shield_index,
		 unsigned const int upgrade_value, galaxy_t **galaxy)
{
	...
	if (shield_index < 0 ||
		shield_index >= ((only_data_t *)start->data)->shields_number) {
...
void ADD_KILLS(unsigned int planet_index, galaxy_t **galaxy)
{
	...
	if (planet_index < 0 ||
		(unsigned int)planet_index > (*galaxy)->galaxy_size) {
		printf("Planet out of bounds!\n");
		return;
	}
...
void EXP(unsigned int planet_index, unsigned int shield_value,
		 galaxy_t **galaxy)
{
	// Check the received input
	if (!*galaxy || !(*galaxy)->head) {
	...
	((only_data_t *)(start->data))->shields_number =
		((only_data_t *)(start->data))->shields_number + 1;
}

// Remove a certain shield from a planet
void RMV(unsigned int planet_index, unsigned int shield_index,
		 galaxy_t **galaxy)
{
...
// Collide two planets
void COL(unsigned int index_planet_1, unsigned int index_planet_2,
		 galaxy_t **galaxy, int *global_size)
{
	// Check the received data
	if (!*galaxy) {
	...
	if (my_shield1 < 0 && my_shield1 != -5 && my_shield2 < 0 &&
		my_shield2 != -5) {
...
void ROT(unsigned int planet_index, const char direction, int units,
		 galaxy_t **galaxy)
{
	// Check the received data
	if (!*galaxy) {
	...
	if (direction == 't') {
		for (int i = 0; i < nr_rotations; i++)
			((only_data_t *)(new->data))->shield->head =
				((only_data_t *)(new->data))->shield->head->next;
	} else if (direction == 'c') {
		for (int i = 0; i < nr_rotations; i++)
			((only_data_t *)(new->data))->shield->head =
				((only_data_t *)(new->data))->shield->head->prev;
	}
}
...
void SHW(int planet_index, galaxy_t **galaxy)
{
	...
	if (object_to_print && object_to_print->prev &&
		strcmp(((only_data_t *)object_to_print->data)->name,
			   ((only_data_t *)object_to_print->prev->data)->name) != 0)
		printed_left_neighbour = 1;
	if (object_to_print && object_to_print->next &&
		strcmp(((only_data_t *)object_to_print->data)->name,
			   ((only_data_t *)object_to_print->next->data)->name) != 0)
		printed_right_neighbour = 1;
	if (object_to_print->prev && object_to_print->next &&
		strcmp(((only_data_t *)object_to_print->prev->data)->name,
			   ((only_data_t *)object_to_print->next->data)->name) == 0)
	...
	if (!*galaxy) {
		...
		*galaxy = NULL;
		...
		if (tmp == 1) {
			...
			galaxy_object *start =
				((only_data_t *)((*galaxy)->head->data))->shield->head->prev;
			while (start !=
				   ((only_data_t *)((*galaxy)->head->data))->shield->head) {
				prev = start;
				start = start->prev;
				free(prev->data);
				prev->next = NULL;
			...
			*galaxy = NULL;
			...
			for (int i = 0; i < global_size; i++) {
				...
				galaxy_object *prev = NULL;
				galaxy_object *start =
					((only_data_t *)(prevplanet->data))->shield->head->prev;

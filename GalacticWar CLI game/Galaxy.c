...
dll_add_nth_node(galaxy_t* list, int planet_index, int shields_number, char* planet_name)
{
    ...
    else if(planet_index==0) {// printf("Create firs222t item!\n");
        // Create the head of the list
        galaxy_object* new_object = NULL;
        new_object = malloc(sizeof(galaxy_object));
        DIE(!new_object, "Couldn't chain planets!\n");
    ...
    // else if(planet_index>=list->galaxy_size) {
    //     galaxy_object* new_node = NULL;
    //     new_node  = malloc(sizeof(galaxy_object));
    //     DIE(!new_node,"Couldn't add to list!\n");
    //     new_node->next = NULL;
    //     new_node->prev = NULL;
    //     new_node->data = malloc(NAME_SIZE);
    //     if(sizeof(int) == NAME_SIZE) 
    //         *((int*)new_node->data) = *((int*)data);
    //     else
...
void
dll_print_ints_left_circular(galaxy_object* start)
{
    if(!start) {
        fprintf(stderr,"Didn't receive node correctly!\n");
        return;
    }
    galaxy_object* tmp = NULL;
    tmp = start;
    while(start != tmp) {
        printf("%d ",*((int*)start->data));
        start = start->prev;
...
void
dll_print_ints_right_circular(galaxy_object* start)
{
    if(!start) {
        fprintf(stderr,"Didn't receive node correctly!\n");
        return;
